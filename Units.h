...
#define IN1 const Unit<l, m, t, i, k, n, j>& rhs
#define IN2 const Unit<L, M, T, I, K, N, J>& rhs
#define FIN const Value & rhs
#define TMPLT template<int l, int m, int t, int i, int k, int n, int j>
#define OTHR_TMPLT template<int L, int M, int T, int I, int K, int N, int J>
#define RET return UnitName <l, m, t, i, k, n, j>::Name
#define B_RET return bool
#define MLT_RET return UnitName <l + L, m + M, t + T, i + I, k + K, n + N, j + J>::Name
#define DIV_RET return UnitName <l - L, m - M, t - T, i - I, k - K, n - N, j - J>::Name
#define INV_RET return UnitName <-l, -m, -t, -i, -k, -n, -j>::Name
#define SQRT_RET UnitName <l / 2, m / 2, t / 2, i / 2, k / 2, n / 2, j / 2>::Name
...
LITERAL(Type, PR_FEMTO##ltr, 1e-15 * coeff)    \
LITERAL(Type, PR_PICO##ltr, 1e-12 * coeff)    \
LITERAL(Type, PR_NANO##ltr, 1e-9 * coeff)     \
LITERAL(Type, PR_MICRO##ltr, 1e-6 * coeff)    \
LITERAL(Type, PR_MILLI##ltr, 1e-3 * coeff)     \
LITERAL(Type, ltr, 1 * coeff)           \
LITERAL(Type, PR_KILO##ltr, 1e+3 * coeff)     \
LITERAL(Type, PR_MEGA##ltr, 1e+6 * coeff)     \
LITERAL(Type, PR_GIGA##ltr, 1e+9 * coeff) \
LITERAL(Type, PR_TERA##ltr, 1e+12* coeff)
...
namespace Units
{
   ...
   {
      typedef Unit<l, m, t, i, k, n, j> Name;
   ...
   template <>
   struct UnitName<0, 0, 0, 0, 0, 0, 0> { typedef Value Name; };
   
   //Base SI Units
   VALUE(Length, m, 1, 0, 0, 0, 0, 0, 0);
   VALUE(Mass, kg, 0, 1, 0, 0, 0, 0, 0);
   VALUE(Time, s, 0, 0, 1, 0, 0, 0, 0);
   VALUE(ElCurrent, A, 0, 0, 0, 1, 0, 0, 0);
   VALUE(Temperature, K, 0, 0, 0, 0, 1, 0, 0);
   VALUE(SubstanceAmount, mol, 0, 0, 0, 0, 0, 1, 0);
   VALUE(LumIntensity, cd, 0, 0, 0, 0, 0, 0, 1);
   //SI derived units
   VALUE(Area, m^2, 2, 0, 0, 0, 0, 0, 0);
   VALUE(Volume, m^3, 3, 0, 0, 0, 0, 0, 0);
   VALUE(Velocity, m/s, 1, 0, -1, 0, 0, 0, 0);
   VALUE(ArealVelocity, m^2/s, 2, 0, -1, 0, 0, 0, 0);
   VALUE(Acceleration, m/s^2, 1, 0, -2, 0, 0, 0, 0);
   VALUE(Force, N, 1, 1, -2, 0, 0, 0, 0);
   VALUE(Charge, C, 0, 0, 1, 1, 0, 0, 0);
   VALUE(Density, m^-3, -3, 0, 0, 0, 0, 0, 0);
   VALUE(Frequency, Hz, 0, 0, -1, 0, 0, 0, 0);
   VALUE(Impulse, kg*m/s, 1, 1, -1, 0, 0, 0, 0);
   VALUE(Pressure, Pa, -1, 1, -2, 0, 0, 0, 0);
   VALUE(Energy, J, 2, 1, -2, 0, 0, 0, 0);
   VALUE(GrPotential, m^2/s^2, 2, 0, -2, 0, 0, 0, 0);
   VALUE(Power, W, 2, 1, -3, 0, 0, 0, 0);
   VALUE(ElPotential, V, 2, 1, -3, -1, 0, 0, 0);
   VALUE(ElResistance, Omh, 2, 1, -3, -2, 0, 0, 0);
   VALUE(ElResistivity, Omh*m, 3, 1, -3, -1, 0, 0, 0);
   VALUE(ElConductance, S, -2, -1, 3, 2, 0, 0, 0);
   VALUE(ElConductivity, S/m, -3, -1, 3, 1, 0, 0, 0);
   VALUE(ElCapacity, F, -2, -1, 4, 2, 0, 0, 0);
   VALUE(ElInductance, H, 2, 1, -2, -2, 0, 0, 0);
   VALUE(ElDipoleMoment, C*m^2, 1, 0, 1, 1, 0, 0, 0);
   VALUE(ElStrength, V/m, 1, 1, -3, -1, 0, 0, 0);
   VALUE(SpecificHeat, J/(kg*k), 2, 0, -2, 0, -1, 0, 0);
   VALUE(WaveNumber, 1/m, -1, 0, 0, 0, 0, 0, 0);
   VALUE(Stiffness, N/m, 0, 1, -2, 0, 0, 0, 0);
   VALUE(InertiaMoment, kg*m^2, 2, 1, 0, 0, 0, 0, 0);
   VALUE(ChargeMassRatio, C/m, 0, -1, 0, 1, 1, 0, 0);
   VALUE(MassDensity, kg/m^3, -3, 1, 0, 0, 0, 0, 0);
   VALUE(ChargeDensity, C/m^3, -3, 0, 1, 1, 0, 0, 0);
   VALUE(CurrentDensity, A/m^2, -2, 0, 0, 1, 0, 0, 0);
   VALUE(VolumetricFlow, m^3/s, 3, 0, -1, 0, 0, 0, 0);
   VALUE(DensityOfFlow, 1/(m^2*s), -2, 0, -1, 0, 0, 0, 0);
   VALUE(Luminance, lx, -2, 0, 0, 0, 0, 0, 1);
   VALUE(MagneticMoment, J/T, 2, 0, 0, -1, 0, 0, 0);
   VALUE(StefansConst, W/(m^2*K^4), 0, 1, -3, 0, -4, 0, 0);
   VALUE(AvogadroConst, 1/mol, 0, 0, 0, 0, 0, -1, 0);
   VALUE(MgStrength, T, 0, 1, -2, -1, 0, 0, 0);
   VALUE(RadExitance, W/m^2, 0, 1, -3, 0, 0, 0, 0);
   VALUE(MolarMass, kg/mol, 0, 1, 0, 0, 0, -1, 0);
   VALUE(DynViscosity, Pl, -1, 1, -1, 0, 0, 0, 0);

   VALUE(FaradayConstant, s*A/mol, 0, 0, 1, 1, 0, -1, 0);
   VALUE(BoltsmanConstant, J/K, 2, 1, -2, 0, -1, 0, 0);
   VALUE(MgConst, V*s/(A*m), 1, 1, -2, -2, 0, 0, 0);
   VALUE(PlanckConstant, J*s, 2, 1, -1, 0, 0, 0, 0);
   VALUE(GrConstant, m^3/(kg*s^2), 3, -1, -2, 0, 0, 0, 0);
   VALUE(GasConstant, J/(K*mol), 2, 1, -2, 0, -1, -1, 0);
   VALUE(ElConst, N*m^2/C^2, -3, -1, 4, 2, 0, 0, 0);
   ...
   TMPLT class Unit : public UnitName<l, m, t, i, k, n, j>
   {
      Value val_ = 0;
      OTHR_TMPLT friend class Unit;
      TMPLT friend inline auto operator * (const Value lhs, IN1);
      TMPLT friend inline auto operator / (const Value lhs, IN1);
      TMPLT friend inline auto sqrt(IN1);
   ...
   public:
      ...
      OTHR_TMPLT OPERATOR(*, MLT_RET(val_ * rhs.val_), const, IN2)
      OTHR_TMPLT OPERATOR(/, DIV_RET(val_ / rhs.val_), const, IN2)
   ...
   TMPLT OPERATOR(*, RET(rhs.val_ * lhs),, const Value lhs, IN1)
   TMPLT OPERATOR(/, INV_RET(lhs / rhs.val_),, const Value lhs, IN1)

   template <bool b>
   struct PowCheck
   {
      template <typename T>
      static auto get(T& ret) { return ret; }
   };
   ...
   {
      template <typename T>
      static auto get(T& ret) { return ret.getSI(); }
   };

   TMPLT inline auto sqrt(IN1)
   {
      return PowCheck<!(l % 2 || m % 2 || t % 2 || i % 2
         || k % 2 || n % 2 || j % 2)>::get(SQRT_RET(sqrtl(rhs.val_)));
   }
